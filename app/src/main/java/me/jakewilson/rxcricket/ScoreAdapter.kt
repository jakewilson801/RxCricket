...
import kotlinx.android.synthetic.main.score_item.view.player_one_score
import kotlinx.android.synthetic.main.score_item.view.player_two_score
import kotlinx.android.synthetic.main.score_item.view.which_target
...
class ScoreAdapter(_bus: RxBus) : RecyclerView.Adapter<ScoreAdapter.ScoreViewHolder>() {
  ...
  val scores: List<ScoreItem> = targets.map { it ->
    ScoreItem(STRIKE_LEVEL.NONE, STRIKE_LEVEL.NONE, it.key.toString(), 0)
  }
  ...
  data class ScoreItem(var strikeP1: STRIKE_LEVEL, var strikeP2: STRIKE_LEVEL, val label: String,
      val score: Int)
  ...
  override fun onCreateViewHolder(parent: ViewGroup,
      itemType: Int): ScoreViewHolder = ScoreViewHolder(
      LayoutInflater.from(parent.context).inflate(R.layout.score_item, parent, false))

  override fun onBindViewHolder(holder: ScoreViewHolder, position: Int) = holder.bindScore(this,
      _bus, ScoreHolder(scores[position], scores))
  ...
  class ScoreViewHolder(view: View) : RecyclerView.ViewHolder(view) {
    fun bindScore(adapter: ScoreAdapter, _bus: RxBus, scoreHolder: ScoreHolder) {
      with(scoreHolder) {
        ...
        val springListenerP1 = DartsSpringListener(adapter, itemView.player_one_score, true, _bus,
            scoreHolder)
        val springListenerP2 = DartsSpringListener(adapter, itemView.player_two_score, false, _bus,
            scoreHolder)
        ...
        val layer1 = (itemView.context.resources.getDrawable(
            R.drawable.chalk_marks) as LayerDrawable)
        val layer2 = (itemView.context.resources.getDrawable(
            R.drawable.chalk_marks) as LayerDrawable)
        ...
        itemView.player_one_score.setOnTouchListener { _, motionEvent ->
        ...
        itemView.player_two_score.setOnTouchListener { _ , motionEvent ->
          when (motionEvent.action) {
    ...
    inner class DartsSpringListener(scoreAdapter: ScoreAdapter, view: View, isPlayerOne: Boolean,
        _bus: RxBus, scoreHolder: ScoreHolder) : SimpleSpringListener() {
      val scoreAdapter = scoreAdapter
      val whichView = view
      val isPlayerOne = isPlayerOne
      val _bus = _bus
      val scoreHolder = scoreHolder
      val targets = mapOf("20" to 20, "19" to 19, "18" to 18, "17" to 17, "16" to 16, "15" to 15,
          "B" to 25)

      override fun onSpringUpdate(spring: Spring) {
        var mappedValue = SpringUtil.mapValueFromRangeToRange(spring.currentValue, 0.0, 1.0, 1.0,
            0.5)
